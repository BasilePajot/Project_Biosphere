...
manhattan <- function(name){
  ...
  filename <- paste0("BLUPS", name, "_ACP.Rdata")
  ...
  for (i in 1:ncol(BLUPSins)) {
    ...
    manhatt <- mlmm.gwas::manhattan.plot(res_mlmm, map = mapins, steps = 1, hideCofactors = FALSE, chrToPlot   = "all", unit = "bp", main = paste("Plot for axis", names(BLUPSins)[i], "on table Insects"))
    # Printing it with a line showing the threshold to determine where to stop
    manhatt
    abline(h = 5, col = "red", lwd = 2)
    # Saving the graph
    dev.print(device = png, file = paste0("Manhattan plot of the p-values for all markers on the axis ",    names(BLUPSins)[i], " of the ACP for ", name, ".png"), width = 1800)
  }
}
```
...
```{r}
manhattan("Oulema")  #ok
manhattan("Aphidiinae") # nope
manhattan("Triticum") #ok
manhattan("Viridiplantae") #nope
manhattan("Ecdysozoa") #nope
manhattan("Thysanoptera") #ok
manhattan("Pythiaceae") #nope
manhattan("Aphididae") #nope
# Try "Oulema", "Aphidiinae", "Triticum", "Viridiplantae", "Ecdysozoa", "Thysanoptera","Pythiaceae", "Aphididae"
```
