...
# Calculating the total number of reads depending on the considered cluster (how many reads are there per cluster?) for each table
...
ACPJF <- PCA(ABONDANCE_JF, scale.unit = TRUE, ncp = 40, graph = TRUE) # PB: ATTENTION à ELAX.101.R1 QUI EST VACHEMENT LOIN QUAND MÊME
...
plot.PCA(ACPJF, axes = c(1, 2), choix = "ind", habillage = 1, xlim = c(-10,30))
...
liste_axe_ACPJA <- which(RES_ACPJA$H2 > 0.10)
liste_axe_ACPJF <- which(RES_ACPJF$H2 > 0.10)
...
ACPHJF <- PCA(ABONDANCE_JF[,OTU_HERI_ACPJF], scale.unit = TRUE, ncp = 40, graph = TRUE) # ELAX.101.R and ELAX.241.R2 are strange
...
plot.PCA(ACPHJF, aces = c(1, 2), choix = "ind", habillage = 13, xlim = c(-10,30))
...
for (i in seq_along(1:(ncol(ABONDANCE_A)-1))) {
  RES_OTUA <- c(RES_OTUA, c(i, as.numeric(Hdeux(ABONDANCE_A[,i], ABONDANCE_A)))) # PB: il y a un soucis sur cette ligne "Error in mlRespMod(fr, REML = REMLpass) : response must be numeric"
}
RES_OTUJA <- c()
for (i in seq_along(1:(ncol(ABONDANCE_JA)-1))) {
  RES_OTUJA <- c(RES_OTUJA, c(i, Hdeux(ABONDANCE_JA[,i], ABONDANCE_JA))) # PB: il y a un soucis sur cette ligne "Error in mlRespMod(fr, REML = REMLpass) : response must be numeric"
}
RES_OTUJF <- c()
for (i in seq_along(1:(ncol(ABONDANCE_JF)-1))) {
  RES_OTUJF <- c(RES_OTUJF, c(i, Hdeux(ABONDANCE_JF[,i], ABONDANCE_JF))) # PB: il y a un soucis sur cette ligne "Error in mlRespMod(fr, REML = REMLpass) : response must be numeric"
}
...
# Plotting the heritability of the abundance depending on the mean of the abundance PB: JE SAIS PAS CE QUE ÇA VEUT DIRE ...
par(mfrow = c(3,1))
...
liste_otuA <- which(RES_OTUA$H2>0.4)   # PBB: Jeter un oeil à valeurs de départ
...
# calculating the heredity of the data frames
...
# CA(res.caA$eig)  # PB: C'est quoi ça ?
# dim(res.ca$eig)  # PB: C'est quoi ça ?

# Making a list of the axis that are the mot heritable in the analysis
liste_axesA <- which(RESA$H2 > 0.1)
liste_axesJA <- which(RESJA$H2 > 0.1)
liste_axesJF <- which(RESJF$H2 > 0.1)
...
names(COHJF)[1:35] <- paste0("D", 1:35)
...
RESHJF <- c()
for (i in 1:40) {
  RESHA <- c(RESHA, c(i, Hdeux(COHA[, i], COHA))) # PBB: il y a à nouveau le message d'erreur (même que celui qui est apparu au niveau du calcul de l'héritabilité sur abondance)
...
# Takes out the selected genotypes in the previous step but takes it in the blups matrix
...
for (i in 1:ncol(BLUPSA)) {
  ...
  abline(h = 5, col = "red")
  # Saving the graph
  dev.print(device = png, file = paste0("Manhattan plot of the p-values for all markers on the axis ",  names(BLUPSA)[i], " of the AFC for table A.png"), width = 600)
}
```

Axes qui ont un QTL - D10; D17; D38; D40
...
for (i in 1:ncol(BLUPSJA)) {
  ...
  abline(h = 5, col = "red")
  # Saving the graph
  dev.print(device = png, file = paste0("Manhattan plot of the p-values for all markers on the axis ",  names(BLUPSJA)[i], " of the AFC for table JA.png"), width = 600)
}
```

Axes qui ont un QTL - D
...
for (i in 1:ncol(BLUPSJF)) {
  ...
  abline(h = 5, col = "red")
  # Saving the graph
  dev.print(device = png, file = paste0("Manhattan plot of the p-values for all markers on the axis ",  names(BLUPSJA)[i], " of the AFC for table JF.png"), width = 600)
}
```
...
manhatt <- mlmm.gwas::manhattan.plot(res_mlmm, map = map, steps = 1, hideCofactors = FALSE,
...
names(snp_max)
bind <- as.data.frame(cbind(y, genot.ok[, names(snp_max)]))
boxplot(bind[, 1]~ bind[, 2])
table(bind[, 2])
anova(lm(bind[, 1]  ~ bind[, 2]))

map[which(map$SNP == names(snp_max)), ]
names(map)
V <-cor(genot.ok[, names(snp_max)], genot.ok)**2
hist(V)
V <- as.data.frame(V)
colnames(V[which(V > 0.8)])
listV <- colnames(V[which(V > 0.8)])

map[map$SNP %in% listV, ]
nrow(map[map$SNP %in% listV, ])
match(listV, map$SNP)
pvaly <- res_mlmm[[2]][listV]
avirer <- cbind(map[match(listV, map$SNP), ], -log10(pvaly))
avirer <- avirer[which(avirer$Chr != "Un"),]

plot(avirer$Pos, avirer[, 4], ylim = c(2, 6))

lessuivants <- map[which(map$Chr == "1B" & map$Pos >= map$Pos[snp_max]), ]
dim(lessuivants)
order(lessuivants$Pos)
lessuivants <- lessuivants[order(lessuivants$Pos), ]
cor(genot.ok[, lessuivants$SNP[1:2]])**2

lesprecedents <- map[which(map$Chr == "1B" & map$Pos <= map$Pos[snp_max]), ]
dim(lesprecedents)
order(lesprecedents$Pos)
lesprecedents <- lesprecedents[order(lesprecedents$Pos, decreasing = ), ]
cor(genot.ok[, lesprecedents$SNP[1:2]])**2
names(map)
rr
