...
hist(y, col = "grey",  nclass=20, xlab = "BLUP ", main=paste("Distribution of", names(BLUPSA)[i]))
genot.okA<-genot.okA[order(rownames(genot.okA)),]
y<-y[order(names(y))]
genot.loop<-genot.okA
...
plot(match(rownames(genot.okA), names(y))) # le résultat de ça c'est pourri
genot.loop<-genot.loop[,mapA$SNP]
plot(match(mapA$SNP, colnames(genot.loop)))
...
Valeurs pour i qui font apparaitre un QTL - D21; D28
