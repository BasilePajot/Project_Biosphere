...
```{r}
library("anyLib")
anyLib(c("tidyverse", "lme4", "SpATS", "viridis", "mlmm.gwas", "FactoMineR"))
...
spatial_dist <- function(x) {
  ...
  summary(sps)
  plot(sps, main = paste0("sPATS of the ", x, " taxa"))
  h <- getHeritability(sps)
  return(transpo)
}

spatial_dist("Oulema")
# Try "Oulema", "Aphidiinae", "Triticum", "Viridiplantae",   "Ecdysozoa", "Thysanoptera","Pythiaceae", "Aphididae"
...
manhattan <- function(name){
  ...
  names(CO_ACPins)[1:(ncol(CO_ACPins) - 2)] <- paste0("D", 1:(ncol(CO_ACPins) - 2))
  
  RES_ACPins <- c()
  for (i in 1:(ncol(CO_ACPins) - 2)) {
    RES_ACPins <- c(RES_ACPins, c(i, Hdeux(CO_ACPins[,i], CO_ACPins)) )# At each passage, it uses the Hdeux   function defined in the beginning and implements its output into the vector used for this
  }
  RES_ACPins <- as.data.frame(matrix(RES_ACPins, ncol = 3, byrow = TRUE))
  RES_ACPins <- RES_ACPins %>% 
    rename(axe = V1,
           VG = V2,
           VE = V3) %>% 
    mutate(H2 = as.numeric(VG) / (as.numeric(VG) + as.numeric(VE)))
  
  liste_axe_ACPins <- which(RES_ACPins$H2 >= 0)
  ...
  BLUPSins <- data.frame()
  for (i in 1:(ncol(CO_ACPins) - 2)) { # We will be calculating blups for the 40 axis of the PCA PB: should   we not use only the "good" ones ?
    # Calculating the blups for each axis
    bluins <- as.data.frame(blups(CO_ACPins[,i], CO_ACPins)) 
    # Naming all the columns as they were named before to be able to recognize them
    names(bluins)[4] <- names(CO_ACPins)[i]
    if (nrow(BLUPSins) == 0) { # Here we separate the first run of the loop from the others.In the fist run,   the BLUPS data frame takes the value of the blups
      BLUPSins <- bluins[,3:4] 
      ...
      BLUPSins <- merge(BLUPSins, bluins[,3:4], by.x = "grp", by.y = "grp")  
    }
  }
  ...
  BLUPSins <- BLUPSins %>%
    rename(EPO = grp) %>% 
    relocate(EPO, .after = last_col())
  # Here we take out the column that have null columns
  blusins <- BLUPSins %>% 
    select(EPO)
  list_names <- c()
  for (i in seq(1, (ncol(BLUPSins) - 1))) {
    if(var(BLUPSins[, i]) != 0){
  ...
  BLUPSins <- blusins %>% 
    as.data.frame()
  for (i in seq(2, ncol(BLUPSins))) {
    names(BLUPSins)[i] <- paste0("D", list_names[i-1])
  }

  # Saving the file of the acquired blups
  filename <- paste0("BLUPS/BLUPS_", name, "_ACP.Rdata")
  ...
  for (i in 1:ncol(BLUPSins)) {
    ...
    list_models <- append(list_models, res_mlmm)
    # Making a Manhattan plot of the mixed model used before
    manhatt <- mlmm.gwas::manhattan.plot(res_mlmm, map = mapins, steps = 1, hideCofactors = FALSE, chrToPlot   = "all", unit = "bp", main = paste("Plot for axis", names(BLUPSins)[i], "on table", name))
    # Printing it with a line showing the threshold to determine where to stop
    manhatt
    abline(h = 5, col = "red", lwd = 2)
    # Saving the graph
    dev.print(device = png, file = paste0("Manhattan_plots/", name, ", Manhattan plot of the p-values for all markers on the axis ", names(BLUPSins)[i], " of the ACP.png"), width = 1800)
    
  }
  # Cleaning up the environment
  #rm(i, ACPins, blusins, bluins, BLUPSins, CO_ACPins, extract, genot.impins, genot.ok, genot.loop, genot.okins, genot.scaled, genot.scaledins, genotins, Kins, mapins, nomsc, RES_ACPins, res_mlmm, SGins, Sins, sins, table_f, filename, list_names, liste_axe_ACPins, mafins, manhatt, name, nomsins, pins, qins, p, q, y, somme, sommeC, somme2)
    return(list_models)
}
```
...
# Trying to find the SNPs that mark the QTLs

```{r}
thys <- manhattan("Thysanoptera")
table_f <- spatial_dist("Thysanoptera") %>% 
    mutate(geno1 = paste(geno, rep, num, way, sep = "_")) %>% 
    select(-c(geno, somme, x, y)) %>% 
    rename(geno = geno1) %>% 
    t() %>% 
...
print(i)
    ...
    genot.scaled <- scale(genot.loop, center = 2 * p, scale = sqrt(2 * p * q))
    res_mlmm <- mlmm_allmodels(y, list(genot.scaled), list(Kins), maxsteps = 4, threshold=1e-4)
mlmm.gwas::manhattan.plot(res_mlmm, map = mapins, steps = 1, hideCofactors = FALSE,
chrToPlot = "all", unit = "bp")
abline(h = 5, col = "red", lwd = 2)
    
hist(res_mlmm[[2]])
if (max(-log10(res_mlmm[[2]])) > 5){
...
mapins[which(mapins$SNP == names(snp_max)), ]
names(mapins)
...
mapins[mapins$SNP %in% listV, ]
nrow(mapins[mapins$SNP %in% listV, ])
match(listV, mapins$SNP)
pvaly <- res_mlmm[[2]][listV]
avirer <- cbind(mapins[match(listV, mapins$SNP), ], -log10(pvaly))
avirer <- avirer[which(avirer$Chr == "6A"),]

plot(avirer$Pos, avirer[, 4], ylim = c(8, 10))

lessuivants <- mapins[which(mapins$Chr == "6A" & mapins$Pos >= mapins$Pos[snp_max]), ]
dim(lessuivants)
