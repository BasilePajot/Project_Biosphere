...
spatial_dist <- function(name) {
  ...
  if (a == 1){
    plot(sps, main = paste0("SpATS of the ", name, " taxon"))
  }else{
    for (i in 1:length(name)){
      if (i == 1){
        titre <- name[i]
      }else{
        titre <- paste(titre, name[i], sep = "-")
      }
    }
...
manhattan <- function(name){
  ...
  ABONDANCE_ins1 <- ABONDANCE_ins %>% 
    as.data.frame() %>% 
    filter(!is.na(ABONDANCE_ins[, 1]))
  ABONDANCE_ins1$somme <- rowSums(ABONDANCE_ins1)
  ABONDANCE_ins1 <- ABONDANCE_ins1 %>% 
    filter(somme != 0) %>% 
    select(-somme)
  ACPins1 <- PCA(ABONDANCE_ins1, scale.unit = TRUE, graph = TRUE)
  if (length(name) == 1) {
    dev.print(device = png, file = paste0("Results/", name, "/PCA of variables without zeros.png"), width = 500)
    ACPins <- PCA(ABONDANCE_ins, scale.unit = TRUE, graph = TRUE)
    dev.print(device = png, file = paste0("Results/", name, "/PCA of variables with zeros.png"), width = 500)
    }else{
      for (j in 1:length(name)){
        if (j == 1){
          titre <- name[j]
      ...
      dev.print(device = png, file = paste0("Results/", titre, "/PCA of variables without zeros.png"), width = 500)
    ACPins <- PCA(ABONDANCE_ins, scale.unit = TRUE, graph = TRUE)
      dev.print(device = png, file = paste0("Results/", titre, "/PCA of variables with zeros.png"), width = 500)
    }  
  ...
  CO_ACPins1 <- CO_ACPins1 %>% 
    ...
    relocate(c(BLOC, geno), .after = last_col())
  
  CO_ACPins <- CO_ACPins1 %>% 
    select(-c(level, way, L.R, num, Jour_sonic, Plaque.extraction, Date.Extraction, Date.Purification..KingFisher, code, x, y, R, C, geno)) 
  CO_ACPins <- CO_ACPins%>% 
    filter(!is.na(CO_ACPins[, 1]))
  
  RES_ACPins <- c()
  for (i in 1:(ncol(CO_ACPins) - 2)){
    RES_ACPins <- c(RES_ACPins, c(i, Hdeux(CO_ACPins[, i], CO_ACPins)))
  }
  
  RES_ACPins <- as.data.frame(matrix(RES_ACPins, ncol = 3, byrow = TRUE))
  names(RES_ACPins) <- c("axe", "VG", "VE")
  ...
  if (length(name) == 1) {
    plot(RES_ACPins$axe, RES_ACPins$H2, xlab = "PCA axis", ylab = "Heritability", main = paste("Heritabilities of the PCA axis on the", name, "taxon"))
    dev.print(device = png, file = paste0("Results/", name, "/Heritability of the PCA axis.png"), width = 500)
    }else{
      for (j in 1:length(name)){
        if (j == 1){
          titre <- name[j]
      ...
      plot(RES_ACPins$axe, RES_ACPins$H2, xlab = "PCA axis", ylab = "Heritability", main = paste("Heritabilities of the PCA axis on the co-occurrence on the", titre, "taxon"))
      dev.print(device = png, file = paste0("Results/", titre, "/Heritability of the PCA axis.png"), width = 500)
    }  
