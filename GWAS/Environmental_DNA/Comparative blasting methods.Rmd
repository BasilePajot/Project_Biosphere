...
reference_taxonomy_LEP <- read_lines("C:/Documents/ECOLE/2021-2022 Montpellier SupAgro/Stage/Stage 2A et césure/Stage CBGP - ARCAD/Données/Data_insects/Taxonomy_NCBI/taxonomy_result_LEP.xml")
```
...
phylogeny_LEP <- reference_taxonomy_LEP[grep("<Lineage>", reference_taxonomy_LEP)]
scientific_name_LEP <- reference_taxonomy_LEP[grep("<ScientificName>", reference_taxonomy_LEP)]
...
list_numbers_names_HEX <- lapply(phylogeny_HEX, function(x){str_count(x, ";")}) %>% 
  unlist()
list_numbers_names_LEP <- lapply(phylogeny_LEP, function(x){str_count(x, ";")}) %>% 
  unlist()
...
cumulative_vector_names_LEP <- c()
cumulative_vector_names_LEP[1] <- c(1)
for (i in 2:length(list_numbers_names_LEP)){ # We start at 2 to be sure that the first scientific name that appears is counted
  cumulative_vector_names_LEP[i] <- list_numbers_names_LEP[i-1] + cumulative_vector_names_LEP[i-1] + 2
}
...
scientific_name_LEP <- scientific_name_LEP[cumulative_vector_names_LEP]
...
names_and_lineage_LEP <- cbind(scientific_name_epurated_LEP, Lineage = phylogeny_LEP)
...
write.table(table_ncbi_HCO, "Taxonomy_NCBI/HCO_ncbi.csv", sep = ";", dec = ".", na = "NA", col.names = TRUE, row.names = FALSE)
write.table(table_ncbi_HEX, "Taxonomy_NCBI/HEX_ncbi.csv", sep = ";", dec = ".", na = "NA", col.names = TRUE, row.names = FALSE)
write.table(table_ncbi_LEP, "Taxonomy_NCBI/LEP_ncbi.csv", sep = ";", dec = ".", na = "NA", col.names = TRUE, row.names = FALSE)

# Cleaning up the environment to prevent it from being overloaded
rm(names_and_lineage_HCO, scientific_name_epurated_HCO, phylogeny_HCO_1, phylogeny_HCO_2, names_and_lineage_HEX, scientific_name_epurated_HEX, phylogeny_HEX, names_and_lineage_LEP, scientific_name_epurated_LEP, phylogeny_LEP)
```
